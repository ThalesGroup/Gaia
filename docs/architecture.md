...
- `crates/gaia-cli`
  ...
  - chatbot template generation helpers
- `crates/gaia-tui`
...
- `templates/chatbot-react`
  - React/Vite chatbot scaffold generated by `gaia init-chatbot`
- `docs`
...
- read/write initialization for commands that intentionally persist config, such as `serve`, `select`, and `init-chatbot`
...
- Rust and npm dependencies are audited in CI and remain blocking
...
- add a backend by implementing `ServingBackend`
- add or update catalog entries in `catalog/models.yaml`
- add deployment targets by reusing `ServeConfig` and backend metadata
- add compatibility tests around the mock API or real backend smoke runs

The preferred direction is to keep backend-specific behavior behind typed helpers and traits, then let the CLI/TUI remain thin orchestration layers.
